...
var page = 'subPageA';
// 1.【require.ensure (4个参数，依次是依赖，回调函数，错误回调-可省略，指定chunk名称)】
//   依赖加载(异步加载)进来并不会执行，在callback中reqiure依赖才会去执行
if (page === 'subPageA') {
  __webpack_require__.e/* require.ensure */(2).then((function() { // 如果第一个参数依赖为空数组，第二个参数require进来的依赖仍然是异步加载进来的
    var subPageA = __webpack_require__(1); // 此处如果不重新require，依赖会打包分割，程序运行时会加载但不会执行
